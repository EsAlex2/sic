            ...
            <table class="w-full">
                <thead>
                    <tr>
                        ...
                        <th class="bg-slate-100 dark:bg-slate-800/60 text-slate-500 dark:text-slate-500 text-[11px] uppercase tracking-wider px-4 py-3 text-left">Turno</th>
                        ...
                        <th class="bg-slate-100 dark:bg-slate-800/60 text-slate-500 dark:text-slate-500 text-[11px] uppercase tracking-wider px-4 py-3 text-left">Asignaturas</th>
                ...
                <tbody>
                    <?php if (empty($cargas)): ?>
                        <tr><td colspan="6" class="px-4 py-8 text-sm text-center text-slate-500 dark:text-slate-500 border-t border-slate-200 dark:border-slate-800/50">No hay cohortes asignadas.</td></tr>
                    <?php else: ?>
                            ...
                            <tr class="hover:bg-slate-50 dark:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-4 py-3 text-sm border-t border-slate-200 dark:border-slate-800/50">
                                    <span class="inline-flex px-2.5 py-0.5 text-xs font-semibold rounded-full bg-purple-500/10 text-purple-400">
                                        <?= htmlspecialchars($c['total_asignaturas']) ?> materia(s)
                                    </span>
                                </td>
...
document.getElementById('buscadorCargas').addEventListener('keyup', function() {
    ...
    rows.forEach(row => {
        // Obtenemos los textos de las columnas relevantes (Sección)
        const seccion = row.cells[1].textContent.toLowerCase();
        
        if (seccion.includes(filter)) {
            row.style.display = '';
        } else {
